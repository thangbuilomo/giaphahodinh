...
function createVerticalCardHtml(person) {
  ...
  const badgeHtml =
    person.title && !person.title.includes("Con nuôi")
      ? `<span style="background:rgba(15,23,42,0.06); padding:2px 6px; border-radius:999px; font-size:8px; line-height:1; white-space:nowrap; flex-shrink:0; align-self:flex-start;">${person.title}</span>`
      : "";
  const deathHtml = deathNote
    ? `<div style="font-size:10px; font-weight:600; line-height:1.3; color:#ef4444; overflow:hidden;">${deathNote}</div>`
    : "";
  ...
  return `
    ...
    >
      <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:10px; min-width:0;">
